    ...
    qwen.channel.assertExchange(exchange, 'direct', {durable: true}, (err) => {
      ...
      qwen.channel.consume(q.queue, (msg) => {
        ...
        parseMessage(msg, (err, parsedMessage) => {
          ...
          qwen.validateMessage(qwen, parsedMessage, err => {
            if (err) {
...
Qwen.prototype.validateMessage = function (qwen, msg, done) {
  ...
  if (schema[exc] && schema[exc][key]) {
    ...
    qwen.log.info({warning: `Publishing message ${exc}:${key} without validation schema`})
    done()
  }
}
